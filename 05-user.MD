...
User Server running or not?
```
systemctl status user
```
User port opened or not?
```
netstat -lntp
```
```
...
```
User process is opened or not?
```
ps -ef | grep user
```
User is able to connect MongoDB db:
```
telnet mongodb.lithesh.shop 27017
```
User is able to connect Redis db:
```
